...
struct Meeting: Identifiable, Codable, Hashable {
    ...
    /// Meeting brief with structured summary, action items, decisions
    var meetingBrief: MeetingBrief?

    /// When the meeting brief was generated
    var briefGeneratedAt: Date?

    /// User dismissed the "name your speakers" prompt
    var speakerNamingDismissed: Bool = false
    ...
    init(
        ...
        speakerNamingDismissed: Bool = false
    ) {
        self.id = id
        self.title = title
        ...
        self.speakerNamingDismissed = speakerNamingDismissed
    }
    ...
    /// Check if we should show the speaker naming prompt
    /// Shows when: has speakers, not all named, and user hasn't dismissed
    var shouldShowSpeakerNamingPrompt: Bool {
        // Don't show if dismissed
        guard !speakerNamingDismissed else { return false }

        // Need speaker data
        guard hasSpeakerData else { return false }

        // Check if any speakers are unnamed (no user-assigned label)
        let speakers = uniqueSpeakers
        guard !speakers.isEmpty else { return false }

        // If all speakers have user-assigned labels, don't show
        let labels = speakerLabels ?? []
        let userAssignedCount = labels.filter { $0.isUserAssigned }.count

        return userAssignedCount < speakers.count
    }
